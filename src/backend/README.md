...
For single-text-modality benchmarks that expect an M2A-style `/add` and `/search` interface, run `lightmem_text_memory_api_server.py` instead of the full streaming API. This is a LightMem-Ego text-only ablation: it keeps `M_st`, `M_lt`, router/planner/fusion, and EM2Mem-style multi-scale text memories, but removes current visual memory, media ingest, ASR, VLM, and image evidence.
...
API details and the exact changes from the official multimodal code are documented in `docs/lightmem_text_ablation.md`.
